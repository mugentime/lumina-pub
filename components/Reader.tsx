...
export const Reader: React.FC<ReaderProps> = ({
  ...
  const [completedChapters, setCompletedChapters] = useState<number[]>(
    book.progress.completedChapters || []
  );
  ...
  const confirmAddBookmark = () => {
    const newBookmark: Bookmark = {
      ...
      scrollPosition: window.scrollY,
      label:
        pendingBookmarkLabel.trim() ||
        `${currentChapter.title} - ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`,
      createdAt: Date.now(),
    };
  ...
  const progressPercent = Math.round(
    (completedChapters.length / book.chapters.length) * 100
  );

  return (
    <div
      className={`min-h-screen transition-colors duration-500 ${getThemeColors()} font-serif`}
    >
      {/* Sticky Header */}
      <div
        ...
        <div className="flex justify-between items-center w-full max-w-5xl mx-auto">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:block">
              <h2 className="font-sans font-medium text-sm truncate max-w-[200px]">
                {book.title}
              </h2>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-gray-100/10 rounded-full p-1 border border-gray-200/20">
              ...
              <span className="text-xs font-sans w-4 text-center">
                {fontSize}
              </span>
              <button
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className={`w-6 h-6 rounded-full border border-gray-300 bg-white ${theme === 'light' ? 'ring-2 ring-indigo-500' : ''}`}
              />
              <button
                onClick={() => setTheme('sepia')}
                className={`w-6 h-6 rounded-full border border-orange-200 bg-[#eaddcf] ${theme === 'sepia' ? 'ring-2 ring-indigo-500' : ''}`}
              />
              <button
                onClick={() => setTheme('dark')}
                className={`w-6 h-6 rounded-full border border-gray-700 bg-gray-900 ${theme === 'dark' ? 'ring-2 ring-indigo-500' : ''}`}
              />
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200/10">
              <h2 className="text-xs font-sans font-bold uppercase tracking-widest opacity-50">
                Navigation
              </h2>
              <button
                onClick={() => setShowSidebar(false)}
                          ...
                          <li key={idx} className="flex items-start gap-3">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                if (isRead) {
                                  const newCompleted = completedChapters.filter(
                                    (c) => c !== idx
                                  );
                                  setCompletedChapters(newCompleted);
                                  onUpdateProgress({
                      ...
                      {bookmarks.length === 0 ? (
                        <div className="text-center text-gray-500 py-10">
                          ...
                          <p className="text-xs mt-1">
                            Tap the bookmark icon in the top right to save your
                            spot.
                          </p>
                        </div>
                      ) : (
                        <ul className="space-y-4">
                          {bookmarks.map((bm) => (
                            <li
                              key={bm.id}
                              className="group flex items-start justify-between gap-2 p-3 rounded-lg hover:bg-gray-100/10 border border-transparent hover:border-gray-200/20"
                            >
                              <button
                                onClick={() => handleJumpToBookmark(bm)}
                                className="text-left flex-1"
                              >
                                <div className="text-sm font-bold font-serif mb-1">
                                  {bm.label}
                                </div>
      ...
      {isBookmarkModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            ...
            <p className="text-sm opacity-60 mb-4 font-sans">
              Enter a label for this bookmark to help you find it later.
            </p>
      ...
      >
        <div className="mb-12 text-center">
          <h3 className="font-sans text-xs uppercase tracking-widest opacity-50 mb-2">
